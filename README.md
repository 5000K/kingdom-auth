...
> ! kingdom-auth is in an alpha state. If it's promises sound enticing to you, make sure to leave a star. But don't use it in production yet. There **WILL** be breaking changes until the 1.0 release.

## About - a manifesto
kingdom-auth is a minimal authentication backend written in go (golang). It aims to provide the simplest dev-experience possible without sacrificing on user experience or security. And without locking you into anything. It's so simple and minimal that migrating away from it is easy, should you ever need or want to.

The single core mission of kingdom-auth is to provide a seamless, quick to set up + integrate way to authenticate your users. It's JWTs are trivial to validate in your own services, it's slim typescript library is quickly integrated into your frontend. Even for things beyond that: implementing a usable kingdom-auth client consists of implementing three endpoints and doing some basic, scheduled token-refreshing in the background.
